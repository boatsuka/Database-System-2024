...
select ProductName , Price , QuantityInStock , VendorName , CategoryName from Products P
...
SELECT P.ProductName, max(OD.Quantity) AS Quantities,P.Price AS PricePerpiece,(max(OD. Quantity) * P.Price) AS Price from OrderDetails OD
...
SELECT YEAR(O.OrderDate) AS YEAR, P.ProductName,SUM(OD.Quantity) AS Quantity ,P.Price AS Priceperpiece,sum(OD.Quantity * P.Price) AS Price from Products P
